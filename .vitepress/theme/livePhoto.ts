export function initLivePhoto() {
  if (typeof window === 'undefined') return

  const livePhotos = document.querySelectorAll('.live-photo')

  livePhotos.forEach(livePhoto => {
    const container = livePhoto.querySelector('.container') as HTMLElement
    const icon = livePhoto.querySelector('.icon') as HTMLElement
    const video = container.querySelector('video') as HTMLVideoElement
    const image = container.querySelector('img') as HTMLImageElement
    const warning = livePhoto.querySelector('.warning') as HTMLElement
